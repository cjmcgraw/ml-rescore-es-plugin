...
if __name__ == '__main__':
    for v in string.hexdigits.lower():
        response = r.post("http://es:9200/example-data/_search", json={
            "query": {"wildcard": {"name": f"{v}*"}}, "rescore": {
                "window_size": 500, "mlrescore": {
                    "model_name": "grpc-server:50051", "context": uuid().hex, "score_mode": "multiply"
                    }
                }
            })
        response.raise_for_status()

        names, ids = zip(*[[r['_source']['name'], int(r['_id'])] for r in response.json()['hits']['hits']])
